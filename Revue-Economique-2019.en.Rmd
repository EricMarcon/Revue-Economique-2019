...
archive: >
  https://doi.org/10.3917/reco.703.0305 (French published version)
...
Ecologists need to measure the *diversity* of a community of living things, composed of several species whose abundances are known.
...
The nomenclature of economic sectors is the NACE (Statistical Classification of Economic Activities in the European Community) in its revision 2. 
...
The sectors are indexed by the letter $s$ and the countries by the letter $i$.
...
To simplify the writing, the level of aggregation corresponding to all sectors will be called "the industry" and that corresponding to all countries will be called "Europe": $n_s$ will thus be called the number of employees working in the sector $s$ in Europe.
...
The notion of information, defined by Shannon, is identical to that of surprise, which is more intuitive.
...
The inverse of the probability, $1/p_s$, will be called *rarity*[^3]: a very rare species has a probability close to 0.
...
This fundamental property allows it to describe the complexity of a system not only by the possible number of its states, but more simply by the relative frequency of its components, giving birth to the theory of information.
...
@Tsallis1988 proposed a generalized entropy in statistical physics for systems that do not meet the properties required by Boltzmann theory. 
It had been defined by @Havrda1967 in cybernetics and then rediscovered, notably by @Daroczy1970 in information theory, hence its name: *HCDT* entropy (see @Mendes2008, page 451, for a complete history).
...
- $q=1$: the entropy is Shannon's. In econometrics, $S-^{1}H$ is the Theil index;
...
The desire of @Hill1973 was to make diversity indices intelligible after @Hurlbert1971's noted paper entitled "The non-concept of species diversity". 
...
The Hill number of order $q$ is the number of equiprobable species giving the same value of entropy as the observed distribution, in other words an *effective number* of species, also called *number equivalent*. 
...
The notion of $\beta$ diversity was introduced by @Whittaker1960 as the degree of differentiation of biological communities.
The question addressed here is the decomposition of diversity from aggregated data (the diversity of economic sectors in Europe) to a more detailed level (by country).
...
@Marcon2014a generalized this result to all orders of the HCDT entropy: $\beta$ entropy is the average over all countries of the generalized Kullback-Leibler divergence between the $\mathbf{p_{s|i}}$ and $\mathbf{p_{s}}$ distributions, itself defined as the average over all sectors of the information gain brought by the knowledge of the disaggregated distribution:
$$^{q}_{\beta}H(\mathbf{P})=\sum_{i}{p_i\sum_{s}{p_{s|i}[\ln_q{(1/p_{s|i})-\ln_q{(1/p_{s})]}}}}$$.

Like $\gamma$ and $\alpha$ entropies, $\beta$ entropy can be transformed into an effective number which is the number of communities of the same weight, with no common species, that would have the same $\beta$ entropy as the actual communities.
The decomposition of diversity is multiplicative: $\gamma$ diversity is the product of $\alpha$ and $\beta$ diversities.
...
The diversity $^{q}D(\mathbf{p_{s,i}})$ of the data set, all sectors and countries combined, is of great interest, especially theoretically for Shannon's entropy [@Faddeev1956; @Baez2011]: it is called joint diversity [@Gregorius2010].
...
It is equal to the both $\beta$ entropies, that of the diversity and that of the valence.
...
Regardless of the order considered, @Gregorius2010 showed that joint diversity provides important additional information about the distribution of abundances that is not captured by the decomposition of diversity already presented.
The example of biodiversity is used here to simplify the presentation.
$alpha$ diversity is the number of equiprobable species in a typical community.
$\beta$ diversity is the number of these typical communities, equiprobable and without common species.
$\gamma$ diversity is the product of the two previous ones, a number of equiprobable species resulting from the assembly of the communities.
Each species appears only in one community in this representation.
The identical replication of the communities does not modify the $\alpha$, $\beta$ and $\gamma$ diversities: this property is actually required for diversity measures [@Hill1973].
...
The first step in understanding the economic phenomena involved is the characterization of concentration.
...
In mathematical terms, $G$ is the $L^2$ distance between the observed distribution of the sector $s$ and its expected distribution [@Haedo2017], that of the industry in general.
...
HCDT entropy allows to unify and extend these approaches.
Concentration, absolute and relative, will be considered first.
...
At large orders, only mostly occupied countries contribute to the valence.
...
The value of concentration is the proportion of countries left behind (in effective numbers).
...
$\beta$ entropy measures the relative concentration, not the relative valence: the $\alpha$ and $\beta$ entropies have fundamentally different properties.
...
This is an effective number of sectors: if all sectors had a valence equal to the effective number of countries $^{q}D(\mathbf{p_{i|s}})$, it would take $^{q}C_{s}$ sectors to obtain an industry with a valence of $^{q}D(\mathbf{p_{i}})$ effective countries.
...
Relative concentration and absolute valence (figure \@ref(fig:uC)) are related: their product is the absolute valence of all sectors, taken as a reference.
...
Finally, @Bickenbach2013 have combined the (absolute) Theil index and the relative Theil index to better describe spatial concentration by applying them to different economic sectors (industry and services).
...
The food industry, C10, gathers almost half of the employees of industry in Iceland.
...
Diversity can be transformed into absolute specialization, as valence was into concentration, to obtain figure \@ref(fig:s).
...
For this reason, @Mori2005, using similar data, chose to test the spatial concentration of establishments and ignore their number of employees.
...
With the data available here, all the profiles presented in figures \@ref(fig:uC) through \@ref(fig:s) are significantly different from each other from order $0.1$ up.
...
The decomposition is valid at all orders of diversity, and presented as profiles (figure \@ref(fig:dd)).

The profiles are plotted on a logarithmic scale because the values are of different orders of magnitude and also because the multiplicative decomposition becomes additive in this form: the height of the joint diversity on the figure is the sum of the heights of $alpha$ and $beta$ diversities and of replication.

The diversity of Europe ($\gamma$, green curve) has already been presented in figure \@ref(fig:cC).
It is very close to the $\alpha$, within-country diversity (blue curve): the effective number of countries ($\beta$, relative specialization, red curve) varies from 1 (at order 0) to `r format(dpES02CMC$TotalBetaDiversity[which(dpES02CMC$Order==2)], digits=2)` at order 2.
