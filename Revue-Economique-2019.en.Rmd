...
journalinfo: Special issue of the Economic Review
archive: >
  The structuring of the economic space: what contributions can a multidisciplinary approach make?
...
Among them, three families of parametrizable functions have become established: the generalized entropy of the inequality literature [@Shorrocks1980], the entropy of @Renyi1961, which was widely used until the 2000s for the measurement of biodiversity, and, more recently, the HCDT entropy detailed here.
...
The concentration is less than 1 for the agro-food industry (C10): `r format(upC$y[which(upC$x==2)]/upC10$y[which(upC10$x==2)], digits=2)` = `r format(upC$y[which(upC10$x==2)], digits=2)` = `r format(upC$y[which(upC$x==2)], digits=2)` = `r format(upC$y[which(upC10$x==2)]/upC10$y[which(upC10$x==2)]/upC10$y[which(upC10$x==2)].
